...
func NewReader(device, deviceType string, resetPin int) (*NFCTagReader, error) {
	return &NFCTagReader{
		tagChannel: make(chan string, 1),
		devAddress: device,
		resetPin:   resetPin,
		deviceType: deviceType,
		logger:     log.StandardLogger().WithField("component", "nfc-reader"),
	}, nil
}
...
Listener:
	for {
		...
		default:
			...
			if err != nil {
				reader.logger.WithError(err).Errorf("Error polling the reader")
				reader.Reset()
				continue
			}
...
func (reader *NFCTagReader) Cleanup() {
	...
	if err != nil {
		reader.logger.WithError(err).Error("Error closing the reader")
	}
}

// Reset Implements the hardware reset by pulling the resetPin low and then releasing.
func (reader *NFCTagReader) Reset() {
	reader.logger.Infof("Resetting the reader")

	// Refer to gpiod docs
	c, err := gpiod.NewChip("gpiochip0")
	if err != nil {
		reader.logger.WithError(err).Error("Error opening the gpio chip")
		return
	}

	pin, err := c.RequestLine(reader.resetPin, gpiod.AsOutput(1))
	if err != nil {
		reader.logger.WithError(err).Error("Error requesting the reset line")
		return
	}
