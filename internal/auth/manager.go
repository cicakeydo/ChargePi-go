...
func NewManager(localAuthRepository LocalAuthListRepository, tagRepository TagRepository) *ManagerV1 {
...
func (t *ManagerV1) GetTag(tagId string) (*types.IdTagInfo, error) {
	...
	if t.localAuthListEnabled {
		logInfo.Infof("Getting the tag from local auth list")
...
CheckCache:
	// Check the cache
	if t.authCacheEnabled {
		logInfo.Infof("Getting the tag from cache")
