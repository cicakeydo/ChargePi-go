...
func NewChargePoint(evseManager manager.Manager, settingsManager settings.Manager, tagManager auth.Service, sessionManager sessions.Service, diagnosticsManager diagnostics.Service, opts ...chargePoint.Options) (*ChargePoint, error) {
	cp := &ChargePoint{
		state: State{
...
func (cp *ChargePoint) Connect(ctx context.Context, serverUrl string) error {
	...
	wsClient, err := chargePoint.CreateClient(cp.connectionSettings, pingInterval)
	if err != nil {
	...
	logInfo.Infof("Trying to connect to the central system: %s", serverUrl)
	...
	logInfo.Infof("Successfully connected to: %s", serverUrl)
	cp.bootNotification()
	return nil
}
...
func (cp *ChargePoint) Cleanup(reason core.Reason) error {
	...
	if !util.IsNilInterfaceOrPointer(cp.display) {
		...
		if err != nil {
			cp.logger.WithError(err).Error("Error cleaning up display")
		}
	}
	...
	cp.logger.Infof("Disconnecting from the central system")
	cp.chargePoint.Stop()
